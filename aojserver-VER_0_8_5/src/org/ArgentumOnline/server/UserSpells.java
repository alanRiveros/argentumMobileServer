...
public class UserSpells implements Constants {
			...
			if (npc.getAmo() != null
				...
				if (!npc.getAmo().esCriminal()) {
					if (client.getFaccion().ArmadaReal) {
						...
						client.enviarMensaje("Debes desactivar el seguro para atacar a esta mascota.",
							FontType.WARNING);
					}
					return false;
				}
	...
	private boolean hechizoInvocacion() {
		if (client.m_cantMascotas >= MAXMASCOTAS) {
			client.enviarMensaje("No puedes invocar mas mascotas!", FontType.INFO);
			return false;
		}
		boolean exito = false;
		WorldPos targetPos = new WorldPos(client.getFlags().TargetMap,
