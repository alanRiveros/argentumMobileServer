...
import org.ArgentumOnline.server.protocol.ServerCommands;
 ...
 * @author gorlok
 */
public class Client extends BaseCharacter implements Constants, ServerCommands {
	 ...
	 */
	public void enviar(ClientMessage msg, Object... params) {
		if (m_saliendo) {
			return;
		}
		try {
	...
	public void doEnviarMiniEstadisticas() {
		// "MEST" >>>>>> Mini Estadisticas
		enviar(MSG_MEST, m_faccion.CiudadanosMatados,
			m_faccion.CriminalesMatados, m_estads.usuariosMatados,
			m_estads.NPCsMuertos, m_clase.getName(),
			m_counters.Pena);
	}
	...
	private void updateVentanaBanco(short slot, int npc_inv) {
		enviar(MSG_BANCOOK, slot, npc_inv);
	}
	...
	private void iniciarDeposito() {
		...
		enviarEstadsUsuario();
		// Mostramos la ventana para comerciar y ver ladear la osamenta. jajaja
		enviar(MSG_INITBANCO);
	...
	private void sendBanObj(short slot, InventoryObject obj_inv) {
		if (obj_inv != null) {
			...
			enviar(MSG_SBO, slot, info.ObjIndex, info.Nombre, obj_inv.cant,
				info.GrhIndex, info.ObjType, info.MaxHIT, info.MinHIT,
				info.MaxDef);
		} else {
			enviar(MSG_SBO, slot, 0, "(None)", 0, 0);
		}
	}
	...
	public void doComprar(String s) {
		// Comando COMP
		// ¿Esta el user muerto? Si es asi no puede comerciar
		if (!estaVivo()) {
			enviarMensaje("¡¡Estas muerto!!", FontType.INFO);
	...
	public void doEnviarAtribs() {
		// Comando ATRI
		enviar(MSG_ATR, m_estads.getAtribs());
	}

	public void doEnviarFama() {
		enviar(MSG_FAMA, (long) m_reputacion.asesinoRep,
			(long) m_reputacion.bandidoRep,
			(long) m_reputacion.burguesRep,
			(long) m_reputacion.ladronRep,
			(long) m_reputacion.nobleRep,
			(long) m_reputacion.plebeRep, (long) m_reputacion
				.getPromedio());
	}
	...
	public void doUK(String s) {
		...
		switch (val) {
		case SKILL_Robar:
			enviar(MSG_T01, SKILL_Robar);
			break;
		case SKILL_Magia:
			enviar(MSG_T01, SKILL_Magia);
			break;
		case SKILL_Domar:
			enviar(MSG_T01, SKILL_Domar);
			break;
	...
	public void doLanzarHechizo(String s) {
		// Comando LH
		// Lanzar hechizo
		if (!estaVivo()) {
			enviarMensaje("¡¡Estas muerto!!.", FontType.INFO);
	...
	public void enviarCP() {
		...
		mapa.enviarATodos(MSG_CP, m_id, m_infoChar.getCuerpo(),
			m_infoChar.getCabeza(), m_infoChar.getDir(),
			m_infoChar.getArma(), m_infoChar.getEscudo(),
			m_infoChar.getFX(), m_infoChar.getLoops(),
			m_infoChar.getCasco());
	}
		...
		try {
			...
			if (wasLogged) {
				try {
					...
					enviarMensaje("Cerrando la conexión.", FontType.WELLCOME);
					enviarMensaje("Adios. Gracias por jugar con nosotros.",
						FontType.WELLCOME);
					enviar(MSG_FINOK);
				} catch (Exception e) {
					e.printStackTrace();
	...
	public void tirarDados() {
		m_estads.userAtributos[0] = (byte) (Util.Azar(1, 6)
				+ Util.Azar(1, 6) + Util.Azar(1, 6));
		m_estads.userAtributos[1] = (byte) (Util.Azar(1, 6)
				+ Util.Azar(1, 6) + Util.Azar(1, 6));
		m_estads.userAtributos[2] = (byte) (Util.Azar(1, 6)
				+ Util.Azar(1, 6) + Util.Azar(1, 6));
		m_estads.userAtributos[3] = (byte) (Util.Azar(1, 6)
				+ Util.Azar(1, 6) + Util.Azar(1, 6));
		m_estads.userAtributos[4] = (byte) (Util.Azar(1, 6)
				+ Util.Azar(1, 6) + Util.Azar(1, 6));
		enviar(MSG_DADOS, m_estads.userAtributos[0],
	...
	public void doAtacar() {
		...
		if (!m_flags.ModoCombate) {
			enviarMensaje(
				"No estas en modo de combate, presiona la tecla \"C\" para pasar al modo combate.",
				FontType.INFO);
		} else {
			if (m_inv.tieneArmaEquipada()) {
				if (m_inv.getArma().esProyectil()) {
	...
	public void equiparObjeto(String s) {
		// Comando EQUI
		if (!estaVivo()) {
			enviarMensaje(
	...
	public void enviarObjetoInventario(int slot) {
		// updateUserInv
		// "CSI" Slot ObjIndex Name Amount Equipped GrhIndex ObjType
		// MaxHIT MinHit MaxDef Valor\3
		// "CSI" Slot 0 (None) 0 0
		InventoryObject objInv = m_inv.getObjeto(slot);
		if (objInv != null && objInv.objid != 0) {
			ObjectInfo io = server.getInfoObjeto(objInv.objid);
			enviar(MSG_CSI, slot, objInv.objid, io.Nombre, objInv.cant,
				(objInv.equipado ? 1 : 0), io.GrhIndex, io.ObjType, io.MaxHIT,
				io.MinHIT, io.MaxDef, ((long) (io.Valor / 3)));
		} else {
			// Slot vacio.
			enviar(MSG_CSI, slot, 0, "(None)", 0, 0);
		}
	}
	...
	public void enviarMensaje(String msg, FontType fuente) {
		enviar(MSG_TALK, msg + fuente);
	}

	public void enviarHabla(int color, String msg, short id) {
		enviar(MSG_TALK, color, msg, id);
	}

	public void enviarEstadsUsuario() {
		// SendUserStatsBox
		// Ejemplo: EST20,20,50,50,40,40,0,1,300,0
		// "EST" MaxHP MinHP MaxMAN MinMAN MaxSta MinSta GLD ELV ELU Exp
		enviar(MSG_EST, m_estads.MaxHP, m_estads.MinHP,
			m_estads.MaxMAN, m_estads.MinMAN, m_estads.MaxSta,
			m_estads.MinSta, m_estads.oro, m_estads.ELV,
			m_estads.ELU, m_estads.Exp);
	}
	...
	private void cambiarMapa(short mapa, short x, short y, boolean conFX,
		...
		if (m.entrar(this, pos_libre.x, pos_libre.y)) {
			...
			if (enviarData) {
				enviarCambioMapa(mapa);
			}
			// Me envio que ingrese al mapa.
			enviarCC();
			if (conFX) {
				enviarCFX(1, 0);
	...
	public void hablar(int color, String texto, int quienId) {
		...
		Map mapa = server.getMapa(m_pos.mapa);
		if (mapa != null) {
			mapa.enviarATodos(MSG_QDL, quienId);
			mapa.enviarATodos(MSG_TALK, color, texto, quienId);
		}
	}
	...
	public Object[] ccParams() {
		Object[] params = { m_infoChar.getCuerpo(),
				...
				m_infoChar.getEscudo(), m_infoChar.getFX(), 999,
	...
	public void enviarObjeto(int objId, int x, int y) {
		int grhIndex = server.getInfoObjeto(objId).GrhIndex;
		enviar(MSG_HO, grhIndex, x, y);
	}

	public void enviarBQ(int x, int y, boolean bloqueado) {
		enviar(MSG_BQ, x, y, (bloqueado ? 1 : 0));
	}
	...
	public void enviarSkills() {
		...
		enviar(MSG_SKILLS, skills.toArray());
	}
	...
	public void connectNewUser(String nick, String clave, short raza, short genero, CharClass clase,
			byte skills[], String email, short hogar) {
		...
		if (!m_estads.skillsValidos()) {
			Log.serverLogger().severe(
				"[HACK] " + m_nick + " intentó hackear los skills.");
			doSALIR();
			return;
		}
		m_password = clave;
	...
	public void connectUser(String nick, String passwd) {
		...
		try {
			...
			// Call SendData(ToIndex, UserIndex, 0, "ERREl servidor ha alcanzado
			// el maximo de usuarios soportado, por favor vuelva a intertarlo
			// mas tarde.")
			...
			// Call SendData(ToIndex, UserIndex, 0, "ERRNo es posible usar mas
			// de un personaje al mismo tiempo.")
			// Call CloseSocket(UserIndex)
			// Exit Sub
			// End If
			// End If
			...
			enviarEstadsUsuario();
			enviarEstadsHambreSed();
			if (server.estaHaciendoBackup()) {
				enviar(MSG_BKW);
				this
					.enviarMensaje(
						"Por favor espera algunos segundo, WorldSave esta ejecutandose.",
						FontType.INFO);
			}
			...
			Log.serverLogger().info(m_nick + " conectado.");
			// MapInfo(m_pos.Map).NumUsers =
			// MapInfo(m_pos.Map).NumUsers + 1
			if (m_estads.SkillPts > 0) {
				enviarSkills();
				enviarSubirNivel(m_estads.SkillPts);
			}
			// if NumUsers > DayStats.MaxUsuarios Then DayStats.MaxUsuarios =
			// NumUsers
			// If NumUsers > recordusuarios Then
			// Call SendData(ToAll, 0, 0, "||Record de usuarios conectados." &
			// "Hay " & NumUsers & " usuarios." & FONTTYPE_INFO)
			// recordusuarios = NumUsers
			// Call WriteVar(IniPath & "Server.ini", "INIT", "Record",
			// str(recordusuarios))
			...
			if (m_cantMascotas > 0) {
				for (int i = 0; i < m_cantMascotas; i++) {
					// /////// fixme - recuperar mis mascotas!
					// if (UserList(UserIndex).MascotasType(i) > 0) {
					// UserList(UserIndex).MascotasIndex(i) =
					// SpawnNpc(UserList(UserIndex).MascotasType(i), m_pos,
					// True, True)
					//
					// If UserList(UserIndex).MascotasIndex(i) <= MAXNPCS Then
					// Npclist(UserList(UserIndex).MascotasIndex(i)).MaestroUser
					// = UserIndex
